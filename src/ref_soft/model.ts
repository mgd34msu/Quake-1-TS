...
Ported from WinQuake/model.c (the ALIAS MODELS, SPRITE MODELS and
Mod_LoadTextures halves) and WinQuake/r_main.c's R_InitTextures (GNU GPL v2
or later).
...
leaving Mod_LoadTextures, Mod_LoadAliasModel and Mod_LoadSpriteModel as
`ModelLoaderHooks` entries. This module is that hook set for the software
renderer, `softModelHooks`, plus R_InitTextures (r_main.c's, not model.c's --
it owns the checkerboard `notexture` texture the hooks object must carry, so
it lives here; U062's r_main.ts calls it again from R_Init for call-order
fidelity and otherwise never touches its return value).
...
- Mod_LoadTextures's `sky` branch imports `R_InitSky` from "./r_sky"
  (r_sky.c, U064, concurrent with this unit and not yet landed): per this
  unit's brief this is the accepted, anticipated absent-sibling failure, not
  something in this file's scope to fix. `bun run check` / any test that
  imports this module currently fails at load time with exactly "Cannot find
  module './r_sky'" until U064 lands; every function below is otherwise
  ready to run.
...
BRUSHMODEL LOADING -- Mod_LoadTextures
...
export function Mod_LoadTextures(mod: ModelT, buffer: Uint8Array, l: LumpT): void {
  ...
  for (let i = 0; i < m.nummiptex; i++) {
    ...
    if (mt.name.startsWith("sky")) R_InitSky(tx);
  }
