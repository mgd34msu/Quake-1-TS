...
Ported from WinQuake/crc.c and WinQuake/crc.h (GNU GPL v2 or later).

Deviations from PORTING.md / the C source:
- The C passes `unsigned short *crcvalue` as an in/out parameter to `CRC_Init`
  and `CRC_ProcessByte`. Ruling: ported as pure functions over a `number`.
  `CRC_Init()` returns the initial value instead of writing through a pointer;
  `CRC_ProcessByte(crcvalue, data)` returns the updated 16-bit value (`& 0xffff`)
  instead of mutating in place. Call sites become `crc = CRC_ProcessByte(crc, b)`.
...
export const CRC_INIT_VALUE = 0xffff;
export const CRC_XOR_VALUE = 0x0000;
...
export function CRC_ProcessByte(crcvalue: number, data: number): number {
  return ((crcvalue << 8) ^ crctable[(crcvalue >> 8) ^ data]!) & 0xffff;
}
