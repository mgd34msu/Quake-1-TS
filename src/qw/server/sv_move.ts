...
Ported from QW/server/sv_move.c (GNU GPL v2 or later).

sv_move.c -- monster movement

QW/server/sv_move.c differs from WinQuake/sv_move.c in exactly two places
(`diff -w`): the `#include` line (`qwsvdef.h` instead of `quakedef.h`) and
the removal of SV_MoveToGoal's `#ifdef QUAKE2` arm (the QUAKE2-only `enemy`
local plus its `SV_CloseEnough (ent, enemy, dist)` call). Everything else is
byte-identical, so this module mirrors src/server/sv_move.ts line for line
over the QW host types (QwEdictT, qwpr, QW server.ts's FL_*, QW world.ts's
SV_Move/SV_LinkEdict/SV_PointContents). Sharing the WinQuake module is not
possible: its signatures are typed `EdictT`, a different class with a
different `entvars_t` layout.

Deviations from PORTING.md / the C source (all carried over from
src/server/sv_move.ts, which documents each one against the same C text):
- `PF_changeyaw` is forward-declared inside sv_move.c itself and called
  directly from SV_StepDirection, bypassing the builtin table.
  src/qw/server/pr_cmds.ts (Q012) exports it for exactly this call site.
- `c_yes`/`c_no` (`int c_yes, c_no;` at file scope in the C, referenced only
  ...
  holder object, so tests can observe them without a reassignable
...
- `rand()` (stdlib) has no project-wide helper; a file-private `rand()` =
  `Math.floor(Math.random() * 0x8000)` matches stdlib rand()'s [0, RAND_MAX]
  range with RAND_MAX = 0x7fff on the reference toolchain.
- `enemy == sv.edicts` / `!= sv.edicts` (comparing an edict pointer against
  edict 0, the world edict, C's "no entity" sentinel) becomes
  `... === EDICT_NUM(0)` / `!== EDICT_NUM(0)`.
- `(int)ent->v.flags & FLAG` truncations used as booleans each get an
  explicit `!== 0` per PORTING.md's "C truthiness on ints/pointers made
  explicit" rule.

No `#ifdef` branches remain in QW/server/sv_move.c to drop: the QUAKE2 arm
WinQuake carries is already absent from the QW source.
*/

import { PF_changeyaw } from "./pr_cmds";
import { type QwEdictT, qwpr, PROG_TO_EDICT, EDICT_TO_PROG, EDICT_NUM, G_FLOAT } from "./progs";
...
function requireGlobalStruct(): QwGlobalVars {
  if (qwpr.global_struct === null) throw new SysError("sv_move: progs not loaded");
  return qwpr.global_struct;
}

function requireGlobals(): { f: Float32Array; i: Int32Array } {
  if (qwpr.globals === null) throw new SysError("sv_move: progs not loaded");
  return qwpr.globals;
}
...
=============
*/
export function SV_CheckBottom(ent: QwEdictT): boolean {
  const mins = vec3();
  const maxs = vec3();
  const start = vec3();
  const stop = vec3();
...
=============
*/
export function SV_movestep(ent: QwEdictT, move: Vec3, relink: boolean): boolean {
  const oldorg = vec3();
  const neworg = vec3();
  const end = vec3();
  let trace: TraceT;
...
======================
*/
export function SV_StepDirection(ent: QwEdictT, yaw: number, dist: number): boolean {
  const move = vec3();
...
======================
*/
export function SV_FixCheckBottom(ent: QwEdictT): void {
...
================
*/
export function SV_NewChaseDir(actor: QwEdictT, enemy: QwEdictT, dist: number): void {
...
======================
*/
export function SV_CloseEnough(ent: QwEdictT, goal: QwEdictT, dist: number): boolean {
  for (let i = 0; i < 3; i++) {
